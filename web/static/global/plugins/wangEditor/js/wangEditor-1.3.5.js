...
$.extend($E, {
    ...
    'expressionConfig': {
        'path': 'http://paodekuai.lifecreatech.com:8888/static/img/face/',
        'fileNames':[0,99],
        'ext':'.gif'
    }
});
    ...
    'command': function(e) {
        ...
        if (ws != null && sendToId != null) {
            if(!$("#" + sendToId).attr("offline")) {
                $("#imageUploadForm").find('input[name="uploadFile"]').click();
            } else {
                alert('不能发送，当前用户已经下线！');
            }
        } else {
            alert('不能发送图片，当前还没有用户连接！');
        }
    }
},
'insertLocation':{
...
'viewSourceCode': {
    ...
    'command': function(e) {
        e.preventDefault();
        $("#quickReply").find("ul").show();
    },
    'blur':function(e) {
        setTimeout(hideQuickReply, 200);
    }
},
...
$.extend($E.fn, {
	'initMenuConfig': function(){
        ...
        this.editorMenuConfig = [
            ...
            ['insertImage', 'insertVideo', 'insertLocation','insertChatImage'],
...
$.extend($E.fn, {
	'commandHooks':{
        ...
        'insertText': function(commandName, commandValue) {
            this.append(commandValue);
        }
    }
});
$.extend($E.fn, {
