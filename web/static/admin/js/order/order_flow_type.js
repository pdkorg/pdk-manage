			...
			$.ajax({
				...
				success:function(data){
					...
					if(data.result=='success'){
						loadOrderData(flowTypeId,templateUnitId,1);
						updateNumTips();
					}
				}
			});
...
function updateNumTips(){
	var typeIds=tree.jstree()._model.data.root.children;
	var templateIds=[];
	for(index in typeIds){
		templateIds = templateIds.concat(tree.jstree().get_node(typeIds[index]).children);
	}

	$.ajax({
		url:"order/flowType/getOrderNums",
		data:{templataUnitIds:templateIds.toString()},
		type:"POST",
		dataType:"json",
		success:function(data){
...
function initFlowTypeTree(type) {
	...
	tree.jstree({
		"core": {
			...
			'data' : {
				'url' : 'order/flowType/treedata/'+type,
				'data' : function (node) {
					return {
						'id' : node.id
					};
	...
	tree.on("changed.jstree", function (e, data) {
		...
		if(currTreeLeafId != "root") {
			...
			if(parentId=="root"){
				flowTypeId = currTreeLeafId;
				templateUnitId = null;
			}else{
				flowTypeId = parentId;
				templateUnitId = currTreeLeafId;
			}
			loadOrderData(flowTypeId,templateUnitId,1);
		}else{
			//清空右边数据
...
/**
 *
 * @param typeId 暂时不传值，后面如果有需求再使用
 * @param unitId
 * @param pageNum
 */
function loadOrderData(typeId,unitId,pageNum){

	$.ajax({
		url:"order/flowType/queryOrderData",
		data:{
			flowTypeId:'',templateUnitId:unitId,pageNum:pageNum
		},
		dataType:"json",
		type:"POST",
		success:function(data){
...
function prePage(){
	...
	loadOrderData(flowTypeId,templateUnitId,parseInt(pageNum)-1);
}
function nextPage(){
	var pageNum = $("#currentPage")[0].getAttribute("data-pageNum");
...
function generateOrderHtml(listData){
	...
	$(listData).each(function(index,ele){
		...
		if(head.status!='3' && head.status!='4'){
			divHtml = divHtml.concat('<button class="btn blue-sharp" type="button" style="float: right; margin-top: 4px;" data-orderid="'+head.id+'" data-ts="'+head.tsIden+'" data-flowAction="'+head.flowAction+'" data-flowunit="'+head.deliveryStatus+'">');
